...
function reduce(state, action) {
  ...
  switch (action.type) {
    ...
    case 'editar':
      return state.map((contacto) => contacto.id === contacto.id ? contacto : contacto);
    case 'borrar':
      return state.filter((contacto) => contacto.id !== contacto);
    default:
      return state;
  }
}  
...
function App() {
  ...
  function alActualizar(nuevo) {
    if (nuevo != null) {
      if (nuevo.id === 0) {
        dispatch({ type: 'agregar', payload: { ...nuevo, id: generarId() } });
      } else {
